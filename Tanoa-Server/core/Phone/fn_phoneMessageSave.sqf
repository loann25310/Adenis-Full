...

{
	...
	if (!(_new_messages isEqualTo [])) then
	{
		...
		{
			_old_messages = ([(_queryResult select 0)] call AdenisServer_fnc_mresArray);

			{
				_old_messages pushBack _x;
			} forEach _new_messages;

			[format["UPDATE players SET PHONE_messages='%1' WHERE PHONE_number='%2'", [_old_messages] call AdenisServer_fnc_mresArray, _x], 1] call ExtDB3_fnc_async;
		};
	};
