...
if (!(isNil "_vInfo")) then
{
	[
		...
		[
			...
			((fuel _vehicle) * 100),
			if (_storeInventory) then {[(_vehicle getVariable ["Trunk", []])] call AdenisServer_fnc_mresArray} else {"""[]"""},
			(_storePos select 0),
			(_storePos select 1),
			(_storePos select 2),
			(_vInfo select 0),
			(_vInfo select 2),
			[([_vehicle] call AdenisClient_fnc_getAllHitPointsDamage) select 2] call AdenisServer_fnc_mresArray,
