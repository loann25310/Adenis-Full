...
if (!(_target getVariable ["restrained", false]) && !(_target getVariable ["surrender", false])) exitWith {
	["La cible doit avoir les mains sur la tête ou être menottée pour pouvoir effecter un test ADN."] call AdenisClient_fnc_error;
};

if (!([false, "adn", 1] call AdenisClient_fnc_handleInv)) exitWith {
	["Vous n'avez pas de test ADN."] call AdenisClient_fnc_error;
};

if (!(["Test ADN", 30, _target] call AdenisClient_fnc_showProgress)) exitWith {};
...
[
	...
	[(getPlayerUID _target)] call AdenisClient_fnc_adn_get
];
