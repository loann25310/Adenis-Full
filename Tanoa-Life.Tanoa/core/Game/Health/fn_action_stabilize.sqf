...
if ((_type isEqualTo "defib") && !("SkylineItems_Defibrilateur" in (magazines player))) exitWith {
	["Vous n'avez pas de défibrilateur."] call AdenisClient_fnc_error;
};
if ((_type isEqualTo "defib") && (getNumber(missionConfigFile >> "ADENIS_FACTIONS" >> str(playerSide) >> "can_use_debrif") isEqualTo 0)) exitWith {
	["Vous n'êtes pas abilité à manier ce type de défibrilateur."] call AdenisClient_fnc_error;
};
if (_unit getVariable ["bullet_check", false]) exitWith {
	["Cette personne a été gravement blessée et doit être amenée à un centre hospitalier pour être opéré."] call AdenisClient_fnc_error;
};
if (_unit getVariable ["bullet_operation_inUse", false]) exitWith {
	["Cette personne est en train d'être opéré et ne peut pas être stabilisé pour le moment"] call AdenisClient_fnc_error;
};
...
if (isNil "_chance") exitWith {
	["Impossible de déterminer vos chances de réaminer"] call AdenisClient_fnc_error;
};
...
while {(!g_interrupted && (_unit getVariable ["is_coma", false]) && !(player getVariable ["is_coma", false]) && !(player getVariable ["restrained", false]) && (player distance _unit < 3))} do
{
	...
	if (_type isEqualTo "defib") then {
		[player, "defib", 20] call AdenisClient_fnc_globalSay3d;
	};
