...
if (_unit getVariable ["bed_awake", false]) exitWith {
	["La cible est déjà installée."] call AdenisClient_fnc_error;
};

_bed = (nearestObjects [(getPosATL _unit), _classes, 2.8]) select 0;
if (isNil "_bed") exitWith {
	["Vous n'êtes près d'aucun support médical."] call AdenisClient_fnc_error;
};
...
_reader = switch typeOf(_bed) do
{
	...
	case "Alysia_Medical_Lit_01_F":
	{
		...
		if (_unit getVariable ["bullet_check", false]) then {
			["Le patient a besoin d'être opéré d'urgence sur une table d'opération."] call AdenisClient_fnc_info;
		} else {
			[_bed] remoteExec ["AdenisClient_fnc_action_hospital_awake", _unit];
		};
		true;
	};
};
...
if (_reader) then
{
	...
	if (!(isNil "_reader")) then {
		[_reader, _bed] remoteExec ["AdenisClient_fnc_action_hospital_heartReader", _unit];
	};
};
