...
if (g_company != _target) exitWith {
	["Vous ne faites pas parti de cette entreprise."] call AdenisClient_fnc_error;
};
...
if (_target getVariable ["launder", false]) exitWith {
	["Cette entreprise est déjà en train de blanchir de l'argent."] call AdenisClient_fnc_error;
};
...
if ([false, "illegal_money", _amount] call AdenisClient_fnc_handleInv) then
{
	...
	[_target, _amount, (player getVariable "realname"), (lbText[70001, lbCurSel 70001])] remoteExec ["AdenisServer_fnc_company_launder", 2];
	[1] call AdenisDB_fnc_query_update_partial;
	[player, _amount, _amount, "COMPANY"] remoteExecCall ["AdenisServer_fnc_logLaunder", 2];
} else {
