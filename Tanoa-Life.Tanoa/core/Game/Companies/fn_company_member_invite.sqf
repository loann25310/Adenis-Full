...
if (count((_company getVariable "company_members") select 0) >= _max) exitWith {
	[format["<t color='#74DF00'>%1</t> comprends déjà %2 employés et ne peut en avoir plus", (_info select 0), _max]] call AdenisClient_fnc_error;
};

if (getNumber(missionConfigFile >> "ADENIS_FACTIONS" >> str(side _who) >> "companies" >> "employee") isEqualTo 0) exitWith {
	["La personne que vous essayez de recruter n'est pas en mesure d'être employée d'une entreprise"] call AdenisClient_fnc_error;
};
...
if ((_item_player != "") && !(_item_player in (magazines player))) exitWith
{
	[format[
		...
		(([_item_player] call AdenisClient_fnc_fetchCfgDetails) select 1),
		(_info select 0)
...
if ((_item_target != "") && !(_item_target in (magazines _who))) exitWith
{
	[format[
		...
		(_info select 0),
		(([_item_target] call AdenisClient_fnc_fetchCfgDetails) select 1)
	]] call AdenisClient_fnc_error;
};

[format["Vous avez proposé à <t color='#045FB4'>%1</t> de rejoindre l'entreprise <t color='#74DF00'>%2</t>.", (_who getVariable ["realname", (name _who)]), (_info select 0)]] call AdenisClient_fnc_info;
[player, _company] remoteExec ["AdenisClient_fnc_company_member_propose", _who];
