...
if (_name isEqualTo "") exitWith {
	["Vous n'avez pas entré le nom de l'entreprise."] call AdenisClient_fnc_error;
};
_bad = [_name, getText(missionConfigFile >> "ADENIS_COMMPAGNIES_INFO" >> "name_allowed")] call AdenisClient_fnc_TextAllowed;
if (_bad != "") exitWith {
	[format["Vous utilisez un caractère interdit dans le nom de l'entreprise (%1).", _bad]] call AdenisClient_fnc_error;
};
if (([_name] call CBA_fnc_strLen) > getNumber(missionConfigFile >> "ADENIS_COMMPAGNIES_INFO" >> "name_max")) exitWith {
	[format["Votre message ne doit pas dépasser %1 caractères.", getNumber(missionConfigFile >> "ADENIS_COMPANIES_TYPES" >> "characters_max")]] call AdenisClient_fnc_error;
};

closeDialog 0;

if (isNull g_interaction_target) exitWith {
	["Impossible de récupérer les informations du propriétaire de l'entreprise"] call AdenisClient_fnc_error;
};

if (!(isNull g_objPut)) exitWith {
	["Vous deployez déjà un élèment"] call AdenisClient_fnc_error;
};
...
playSound "buy";
[false, _price, "Création d'entreprise"] call AdenisClient_fnc_handleATM;
[_type, _object, g_interaction_target, _name] remoteExec ["AdenisServer_fnc_company_insert", 2];
