...
if (_value isEqualTo -1) exitWith {
	["Vous n'avez pas précisé de valeur."] call AdenisClient_fnc_error;
};
if (_value < 0) exitWith {
	["La valeur ne peut pas être négative."] call AdenisClient_fnc_error;
};
if (!((_value >= 0) && (_value <= 70))) exitWith {
	["La valeur doit être comprise entre 0 et 70."] call AdenisClient_fnc_error;
};
