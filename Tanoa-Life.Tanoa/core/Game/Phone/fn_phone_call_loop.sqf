...
while {((missionNamespace getVariable "calling") && (call AdenisClient_fnc_hasPhone))} do
{
	if (([_phone, _c_channel] call TFAR_fnc_GetChannelFrequency) isEqualTo (missionNamespace getVariable "calling_freq")) then
	{
		if (isNull (missionNamespace getVariable "calling_target")) then
		{
			if (player getVariable ["tf_sw_speakers", false]) then {
				[player, "phone_busy", 6] call AdenisClient_fnc_globalSay3d;
			} else {
		...
		{
			if (player getVariable ["tf_sw_speakers", false]) then {
				[player, "phone_tonalite", 6] call AdenisClient_fnc_globalSay3d;
			} else {
			...
			if (isNull (missionNamespace getVariable "calling_target")) then
			{
				missionNamespace setVariable ["calling_target", ([(missionNamespace getVariable "calling_number")] call AdenisClient_fnc_phone_number_to_unit)];
				if (!(isNull (missionNamespace getVariable "calling_target"))) then
				{
					[player, missionNamespace getVariable "calling_freq"] remoteExec ["AdenisClient_fnc_phone_call_receive", (missionNamespace getVariable "calling_target")];
				};
			};
		};
	};
...
[] call AdenisClient_fnc_phone_call_end;
