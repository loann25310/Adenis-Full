...
if (str(playerSide) in getArray(missionConfigFile >> "ADENIS_DYN_MARKERS" >> _marker >> "shown")) then
{
	...
	[format["Le marqueur <t color='#74DF00'>%1</t> a changé d'emplacement.", (markerText _marker)]] call AdenisClient_fnc_info;
} else {
	private "_index";
	...
	if (_index != -1) then
	{
		...
		[format["Le marqueur <t color='#74DF00'>%1</t> n'est plus d'actualité.", (markerText _marker)]] call AdenisClient_fnc_info;
	};
};
