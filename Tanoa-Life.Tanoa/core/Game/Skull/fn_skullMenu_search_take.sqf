...
switch (_data select 0) do
{
	case "money":
	{
		[true, (_data select 1)] call AdenisClient_fnc_handleCash;
	...
	case "virtual":
	{
		...
		_amount = [_item, (_itemInfo select 1), g_carryWeight, g_maxWeight] call AdenisClient_fnc_calWeightDiff;
		if (_amount > 0) then
		{
			[true, _item, _amount] call AdenisClient_fnc_handleInv;
	...
	case "arma":
	{
		if ([(_data select 1), true, false] call AdenisClient_fnc_handleItem) then {
			g_interaction_target_inv deleteAt _sel;
		} else {
			["Récupérattion <t color='#FF4000'>impossible</t>.<br/>Vous n'avez pas assez de place dans votre inventaire ou avez déjà un objet de ce type équipé."] call AdenisClient_fnc_error;
		};
	};
};
