...
[
	...
	getNumber(missionConfigFile >> "ADENIS_PHONE" >> "FORFAITS" >> g_phone_forfait >> "sms_price")
];
...
if ((
	...
	_index = _list lbAdd getText(missionConfigFile >> "ADENIS_FACTIONS" >> "CIV" >> "name");
...
if (isClass(missionConfigFile >> "ADENIS_FACTIONS" >> str(playerSide) >> "phone" >> "central_plate")) then
{
	...
	_list lbSetPicture [_index, getText(missionConfigFile >> "ADENIS_FACTIONS" >> str(playerSide) >> "icon")];
};
...
if (getNumber(missionConfigFile >> "ADENIS_PHONE" >> "FORFAITS" >> g_phone_forfait >> "hide_number") isEqualTo 1) then
{
	[8337, true] call AdenisClient_fnc_tabletShow;
	[8338, true] call AdenisClient_fnc_tabletShow;
	[8341, true] call AdenisClient_fnc_tabletShow;
};
