...
if (_name isEqualTo "") exitWith {
	["Vous n'avez pas entré de nom pour votre contact"] call AdenisClient_fnc_error;
};

_bad = [_name, getText(missionConfigFile >> "ADENIS_PHONE" >> "CONTACTS" >> "characters_allowed")] call AdenisClient_fnc_TextAllowed;
if (_bad != "") exitWith {
	[format["Vous utilisez un caractère interdit dans le nom de votre contact à ajouter (%1)", _bad]] call AdenisClient_fnc_error;
};
if (([_name] call CBA_fnc_strLen) > getNumber(missionConfigFile >> "ADENIS_PHONE" >> "CONTACTS" >> "characters_max")) exitWith {
	[format["Le nom entré pour votre contact ne doit pas dépasser %1 caractères", getNumber(missionConfigFile >> "ADENIS_PHONE" >> "CONTACTS" >> "characters_max")]] call AdenisClient_fnc_error;
};

if (!([_number] call AdenisClient_fnc_isNumber)) exitWith {
	["Le numéro de téléphone entré n'est pas valide"] call AdenisClient_fnc_error;
};
if (([_number] call CBA_fnc_strLen) != 6) exitWith {
	["Un numéro de téléphone doit être composé de six chiffres"] call AdenisClient_fnc_error;
};
if ((count g_phone_contacts) >= getNumber(missionConfigFile >> "ADENIS_PHONE" >> "FORFAITS" >> g_phone_forfait >> "contacts_max")) exitWith {
	["Vous avez déjà atteint votre quota maximum de contacts"] call AdenisClient_fnc_error;
};
...
if (_modify_name || _modify_number) then
{
	if ((_modify_number && (((g_phone_contacts select _index) select 1) isEqualTo _number)) || (_modify_name && (((g_phone_contacts select _index) select 0) isEqualTo _name))) exitWith {
		[format["Vous possédez déjà un contact avec <t color='#FF4000'>%1</t> comme nom et <t color='#FF4000'>%2</t> en numéro", _name, _number]] call AdenisClient_fnc_error;
	};
