...
if ((call AdenisClient_fnc_hasPhone) && !(missionNamespace getVariable ['calling', false]) && ((missionNamespace getVariable ['calling_number', '']) != '')) then
{
	if (missionNamespace getVariable ["calling_hide", false]) then {
		_txt = "<t size='2.5' align='center'>Inconnu</t>";
	} else {
		_name = [(missionNamespace getVariable ["calling_number", ""])] call AdenisClient_fnc_phone_number_to_name;
