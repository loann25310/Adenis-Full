...
_handle = ["STAFF_DATA"] spawn AdenisClient_fnc_tabletApp;
...
if (_cash > 0) then
{
	_index = _list lbAdd format["%1$ (porte-feuille)", [_cash] call AdenisClient_fnc_numberText];
	_list lbSetPicture [_index, "Alysia_Client\Textures\Items_virtual\money.paa"];
};

if (_atm > 0) then
{
	_index = _list lbAdd format["%1$ (compte)", [_atm] call AdenisClient_fnc_numberText];
	_list lbSetPicture [_index, "Alysia_Client\Textures\Items_virtual\money.paa"];
};
