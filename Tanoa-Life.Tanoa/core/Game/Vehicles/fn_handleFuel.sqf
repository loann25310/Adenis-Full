...
if (!isClass(missionConfigFile >> "ADENIS_FUEL" >> _carburant)) exitWith {
	[format["%1 ne correspond pas à un type d'essence défini.", _carburant]] call AdenisClient_fnc_error;
};
if (!isClass(missionConfigFile >> "ADENIS_VEHICLES" >> typeOf(_vehicle))) exitWith {
	[format["%1 n'est pas fini dans ADENIS_VEHICLES.", typeOf(_vehicle)]] call AdenisClient_fnc_error;
};
