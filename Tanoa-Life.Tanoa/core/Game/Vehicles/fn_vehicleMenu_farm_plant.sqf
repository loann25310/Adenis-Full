...
if (_vehicle getVariable ["farm_plant", false]) exitWith
{
	...
	["Plantaison terminé."] call AdenisClient_fnc_info;
};

if (!(isEngineOn _vehicle)) exitWith {
	["Vous devez avoir le moteur démarré pour débuter la procédure de plantaison."] call AdenisClient_fnc_error;
};
if ((driver _vehicle) != player) exitWith {
	["Seul le conducteur peut débuter la procédure de plantaison."] call AdenisClient_fnc_error;
};
if ((_vehicle getVariable ["trunk_in_use_ID", ""]) != "") exitWith {
	["Vous ne pouvez pas commencer la plantaison alors que le coffre est en cours d'utilisation."] call AdenisClient_fnc_error;
};

_vehicle setVariable ["farm_plant", true];
_vehicle setVariable ["trunk_in_use_ID", "FARMING", true];

["Début de la procédure de plantaison dans <t color='#FE9A2E'>deux secondes</t> ..."] call AdenisClient_fnc_info;
uiSleep 2;
["<t color='#01DF01'>Début</t> de la procédure de plantaison."] call AdenisClient_fnc_info;

while {(_vehicle getVariable ["farm_plant", false])} do
{
	if ((driver _vehicle) != player) exitWith {
		["Plantaison terminé<br/>Vous devez rester à la place de conducteur."] call AdenisClient_fnc_error;
	};
	if (!(isEngineOn _vehicle)) exitWith {
		["Plantaison terminé<br/>Le moteur doit rester allumé."] call AdenisClient_fnc_error;
	};
	if ((_vehicle getVariable ["trunk_in_use_ID", ""]) != "FARMING") exitWith {
		["Plantaison terminé<br/>Quelqu'un fouille le coffre."] call AdenisClient_fnc_error;
	};
...

	{
		if (
				...
				(([(_vehicle getVariable ["Trunk", []]), getText(missionConfigFile >> "ADENIS_FARMING_PLANT_OBJETCS" >> getText(_x >> "plant") >> "seed")] call AdenisClient_fnc_itemTrunk) > 0)
			) exitWith
		{
	...
	if (((isNil "_plant") || (isNil "_seed")) && !isNull(g_company)) then
	{
		...
		{
			{
				if (([(_vehicle getVariable ["Trunk", []]), getText(_x >> "seed")] call AdenisClient_fnc_itemTrunk) > 0) exitWith
				{
	...
	if ((isNil "_plant") || (isNil "_seed")) exitWith {
		["Plantaison terminé<br/>Vous n'êtes près d'aucune zone agricole ou n'avez plus de graines dans le coffre."] call AdenisClient_fnc_error;
	};

	if ((nearestObjects [[_pos select 0, _pos select 1, 0], (call g_plants), _distance]) isEqualTo []) then
	{
		if ([false, _vehicle, "Trunk", _seed, 1, false] call AdenisClient_fnc_handleTrunk) then
		{
			[
				_plant,
				_pos,
				[false, _vehicle, "Trunk", "engrais", 1, false] call AdenisClient_fnc_handleTrunk
			] call AdenisClient_fnc_plantCreate;
			titleText[format["Vous avez planté : 1x %1", [_seed] call AdenisClient_fnc_itemGetName], "PLAIN"];
		};
	};
