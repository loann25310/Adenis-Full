...
[
	...
	[
		[
			"refuel",
			"Plein",
			"[g_interaction_target, (missionNamespace getVariable ['refuel_pump', objNull])] spawn AdenisClient_fnc_fuelStation_refuel_progress;",
			"
			...
			"stock_refuel",
			"Stock",
			"[g_interaction_target, (missionNamespace getVariable ['refuel_pump', objNull])] spawn AdenisClient_fnc_fuelStation_stock_progress;",
			"
				((vehicle player) isKindOf 'Man') &&
				((player getVariable ['stock_station_type', '']) != '') &&
				!(isNull (missionNamespace getVariable ['refuel_pump', objNull])) &&
				(['company_fuel'] call AdenisClient_fnc_hasLicense) &&
				{([(g_interaction_target getVariable ['Trunk', []]), getText(missionConfigFile >> 'ADENIS_FUEL' >> (player getVariable ['stock_station_type', '']) >> 'item_refuel')] call AdenisClient_fnc_itemTrunk) > 0} &&
				!(isNull g_company)
			"
		],[
			"refuel",
			"Jerrican",
			"[g_interaction_target] spawn AdenisClient_fnc_vehicleMenu_refuel_open;",
			"
				((vehicle player) isEqualTo player)	&&
			...
			"siphon",
			"Siphon",
			"[g_interaction_target] spawn AdenisClient_fnc_vehicleMenu_siphon;",
			"
				((vehicle player) isEqualTo player)	&&
				('Alysia_siphon' in (magazines player)) &&
				(['company_garagist'] call AdenisClient_fnc_hasLicense)
			"
		],[
			"lock",
			"Verrouiller",
			"[g_interaction_target, true] call AdenisClient_fnc_vehicleMenu_key;",
			"
				((locked g_interaction_target) != 2) &&
				(
			...
			"unlock",
			"Déverrouiller",
			"[g_interaction_target, false] call AdenisClient_fnc_vehicleMenu_key;",
			"
			...
			"inventory",
			"Coffre",
			"[g_interaction_target] spawn AdenisClient_fnc_virtual_menu_exhange_open;",
			"
				((vehicle player) isKindOf 'Man') &&
				(
					((locked g_interaction_target) != 2) ||
			...
			"repair",
			"Réparation",
			"[g_interaction_target] spawn AdenisClient_fnc_vehicleMenu_repair_open;",
			"
				(vehicle player) isEqualTo player
			"
		],[
			"flip",
			"Retourner",
			"[g_interaction_target] spawn AdenisClient_fnc_vehicleMenu_flip;",
			"
			...
			"flip",
			"Pousser",
			"[g_interaction_target] call AdenisClient_fnc_vehicleMenu_push;",
			"
				(g_interaction_target isKindOf 'Ship') &&
				((vehicle player) isKindOf 'Man') &&
				((locked g_interaction_target) != 2)
			...
			"lockpick",
			"Crochetter",
			"[g_interaction_target] spawn AdenisClient_fnc_vehicleMenu_steal;",
			"
			...
			"pullout",
			"Sortir",
			"[g_interaction_target] call AdenisClient_fnc_vehicleMenu_pullout;",
			"
				((vehicle player) isEqualTo player)
			"
		],[
			"owner",
			"Boîte à gants",
			"[g_interaction_target] spawn AdenisClient_fnc_vehicleMenu_owner;",
			"
			...
			"owner",
			"Immatriculation",
			"[g_interaction_target] spawn AdenisClient_fnc_vehicleMenu_plate;",
			"
				((vehicle player) isEqualTo player)
			"
		],[
			"impound",
			"Fourrière",
			"[g_interaction_target] spawn AdenisClient_fnc_vehicleMenu_impound;",
			"
				(
					(playerSide in [east,west]) ||
					(
						(['company_garagist'] call AdenisClient_fnc_hasLicense) &&
						([getPos g_interaction_target] call AdenisClient_fnc_nearFourriere)
					)
				) && (
					(vehicle player) isEqualTo player
				)
			...
			"farm_gather",
			"Récolter",
			"[g_interaction_target] spawn AdenisClient_fnc_vehicleMenu_farm_gather;",
			"
			...
			"farm_plant",
			"Planter",
			"[g_interaction_target] spawn AdenisClient_fnc_vehicleMenu_farm_plant;",
			"
				(getNumber(missionConfigFile >> 'ADENIS_VEHICLES' >> typeOf(g_interaction_target) >> 'tractor') isEqualTo 1) &&
				((vehicle player) isEqualTo g_interaction_target) &&
				((driver g_interaction_target) isEqualTo player)
			...
			"dog",
			"Chercher",
			"[g_interaction_target] spawn AdenisClient_fnc_vehicleMenu_dog;",
			"
				!(isNull g_dog) &&
			...
			"money_transfer",
			"Prendre sac",
			 "[g_interaction_target] spawn AdenisClient_fnc_vehicleMenu_money_bag_take;",
			"
				((vehicle player) isKindOf 'Man') &&
				(isNull (player getVariable ['wallet_obj', objNull])) &&
				(([(g_interaction_target getVariable ['Trunk', []]), 'money_transfer'] call AdenisClient_fnc_itemTrunk) > 0) &&
			...
			"money_transfer",
			"Déposer sac",
			 "[g_interaction_target] spawn AdenisClient_fnc_vehicleMenu_money_bag_deposit;",
			"
				((vehicle player) isKindOf 'Man') &&
				!(isNull (player getVariable ['wallet_obj', objNull])) &&
				(isClass(missionConfigFile >> 'ADENIS_VEHICLES' >> typeOf(g_interaction_target) >> 'company_money_transfer')) &&
			...
			"money_bomb",
			"Braquer",
			"[g_interaction_target] spawn AdenisClient_fnc_vehicleMenu_bomb;",
			"
				((vehicle player) isKindOf 'Man') &&
			...
			"speaker",
			"Haut-parleurs",
			"[g_interaction_target] spawn AdenisClient_fnc_vehicleMenu_speaker;",
			"
