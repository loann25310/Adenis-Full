...
if ((isNil "_id") || (_target getVariable ["house_sold", false])) exitWith {
	["Ce bâtiment n'est pas acheté."] call AdenisClient_fnc_error;
};
...
if (isNull _storage) then {
	[_target, player] remoteExec ["AdenisServer_fnc_house_init_storage", 2];
} else {
	if ((_storage getVariable ["trunk_in_use_ID", ""]) isEqualTo "") then
	{
		[_target, _storage] remoteExec ["AdenisServer_fnc_house_save_storage", 2];
