...
if (!((g_interaction_target getVariable["house_owner", []]) isEqualTo [])) exitWith {
	["Ce bâtiment est déjà acheté"] call AdenisClient_fnc_error;
};

if (!isNil {(g_interaction_target getVariable "house_sold")}) exitWith {
	["Ce bâtiment a récemment été mis en vente et ne peut pas être achetée de suite"] call AdenisClient_fnc_error;
};
...
if (_actual >= _maxHouse) exitWith {[format["Vous possedez %1 bâtiments. Vous êtes autorisé à en avoir %2.", _actual, _maxHouse]] call AdenisClient_fnc_error};
...
_action =
[
	format["Vous êtes sur le point d'acheter un bâtiment pour <t color='#8cff9b'>%1</t>$", [_price] call AdenisClient_fnc_numberText],
	"Achat de propriété",
	"Acheter",
	"Annuler"
...
if (_action) then
{
	...
	[g_interaction_target, player] remoteExec ["AdenisServer_fnc_house_add", 2];
};
