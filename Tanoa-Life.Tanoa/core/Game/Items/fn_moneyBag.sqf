...
if (!(isNull g_company) && {(((g_company getVariable 'company_info') select 2) isEqualTo 'money_transfer')}) then {
	[_object] spawn AdenisClient_fnc_moneyBagHandle;
} else {
	_action =
	[
	...
	if (_action) then
	{
		if (isNull _object) then {
			["Impossible de trouver le sac. Quelqu'un l'a surement ramasser avant vous."] call AdenisClient_fnc_error;
		} else {
			...
			if (isNull _object) then {
				...
				[true, "illegal_money", 10000] call AdenisClient_fnc_handleInv;
			};
		};
	};
};
