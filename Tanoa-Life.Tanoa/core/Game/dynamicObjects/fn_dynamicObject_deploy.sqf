...
if (!(isNull g_objPut)) exitWith {["Vous deployez déjà un élèment"] call AdenisClient_fnc_error};

_config = (format["getText(_x >> 'item') isEqualTo ('%1')", _item] configClasses (missionConfigFile >> "ADENIS_DYN_OBJECTS")) select 0;
if (isNil "_config") exitWith {["Cet objet ne peut pas être placé"] call AdenisClient_fnc_error};
...
if ([false, _item, 1] call AdenisClient_fnc_handleInv) then
{
	...
	{
		[_object] remoteExec ["AdenisServer_fnc_spikeStrip", 2];
	};
} else {
