...
if (isNil "_data") exitWith {
	["Impossible de récupérer le véhicule à vendre."] call AdenisClient_fnc_error;
};

_price = ([_data select 0] call AdenisClient_fnc_fetchVehInfo) select 19;
_action =
[
	format["Vous êtes sur le point de <t color='#DF0101'>vendre</t> votre véhicule pour <t color='#8cff9b'>%1</t>$.", [_price] call AdenisClient_fnc_numberText],
	"Vente",
	"Valider",
	"Annuler"
...
if (_action) then
{
	...
	{
		playSound "buy";
		[true, _price, "Vente garage"] call AdenisClient_fnc_handleATM;
		g_garage_vehicles deleteAt _index;
		[[nil, nil, (_data select 1), (_data select 2)], false] remoteExec ["AdenisServer_fnc_vehicleDelete", 2];
	};
