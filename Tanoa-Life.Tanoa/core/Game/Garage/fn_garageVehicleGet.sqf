...
_vehicle setVariable ["info", [(getPlayerUID player), (player getVariable ["realname", profileName]), (_data select 1), (_data select 2), (_data select 5)], true];
_vehicle setFuel ((_data select 3) / 100);

/*_plate = (_data select 1);
_vehicle setObjectTextureGlobal [((count getObjectTextures _vehicle)-7), ""];
_a = ((count getObjectTextures _vehicle)-6);
for "_i" from 0 to 6 do {
	_vehicle setObjectTextureGlobal [_a + _i, format ["d3s_cars_core\license\%1.paa", _plate select [_i,1] ]];
};*/
