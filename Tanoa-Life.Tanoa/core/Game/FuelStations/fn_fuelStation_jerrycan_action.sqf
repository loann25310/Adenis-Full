...
if (_type isEqualTo "") exitWith {["Impossible de trouver l'essence que vous avez selectionné."] call AdenisClient_fnc_error};
...
if (player distance _station > getNumber(missionConfigFile >> "ADENIS_FUEL_STATION" >> typeOf(_station) >> "max_distance_allowed")) exitWith {
	["Vous êtes trop loin de la station."] call AdenisClient_fnc_error;
};

if (!("Alysia_jerrycan_empty" in (magazines player))) exitWith {["Vous n'avez pas de jerrycan vide."] call AdenisClient_fnc_error};
...
[player, "action_refuel", 20] call AdenisClient_fnc_globalSay3d;

while {true} do
{
	if (isNull _display) exitWith {
		["Le transfert n'a pas pu aboutir.<br/>Fenêtre fermée."] call AdenisClient_fnc_error;
	};
	if (_bill > g_atm) exitWith {
		["Le transfert n'a pas pu aboutir.<br/>Vous n'avez pas assez d'argent sur votre compte pour payer le plein d'un jerrycan."] call AdenisClient_fnc_error;
	};
	if ((player distance _station) > _distanceBegin) exitWith {
		["Le transfert n'a pas pu aboutir.<br/>Vous êtes trop loin de la station."] call AdenisClient_fnc_error;
	};

	if (_liters isEqualTo 20) exitWith {_fill = true};

	_bill = _bill + ([_station, _type] call AdenisClient_fnc_fuelStation_fuel_getPrice);
	_liters = _liters + 1;
...
if (_fill) then
{
	if (["Alysia_jerrycan_empty", false] call AdenisClient_fnc_handleItem) then
	{
		[getText(missionConfigFile >> "ADENIS_FUEL" >> _type >> "item_jerrycan"), true] call AdenisClient_fnc_handleItem;
		...
		["Le transfert n'a pas pu aboutir.<br/>Vous n'avez pas de jerrycan vide."] call AdenisClient_fnc_error;
	};
} else {
