...
if (isNull _station) exitWith {
	["Impossible de trouver la station."] call AdenisClient_fnc_error;
};

_fuel = _list lbData _sel;
if (_fuel isEqualTo "") exitWith {
	["Impossible de trouver l'essence sélectionnée."] call AdenisClient_fnc_error;
};

(_display displayCtrl 16008) ctrlSetStructuredText parseText format
[
	"<t align='center' size='2'>%1</t>",
	([_station, _fuel] call AdenisClient_fnc_fuelStation_fuel_getPrice)
];

_amount = [_station, _fuel] call AdenisClient_fnc_fuelStation_fuel_getStock;
...
if ((_amount <= 0) && ((buttonAction 16017) isEqualTo "[] spawn AdenisClient_fnc_fuelStation_refuel_action_vehicle;")) then
{
