...
{
	{
		[
			...
			[
				...
				_bill,
				if (getNumber(missionConfigFile >> "ADENIS_VEHICLES" >> typeof(_veh) >> "disableOwnerInfos") isEqualTo 0) then {(_veh getVariable "info") select 2} else {"inconnue"},
	...
	[format["Vous êtes parti sans payer votre facture d'essence s'élevant à <t color='#8cff9b'>%1</t>$.<br/>Votre véhicule a été <t color='#DF0101'>signalé</t>.", [_bill] call AdenisClient_fnc_numberText]] call AdenisClient_fnc_info;
