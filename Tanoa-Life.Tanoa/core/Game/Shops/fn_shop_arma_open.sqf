...
if (!isClass(missionConfigFile >> "ADENIS_SHOPS_ARMA" >> _type)) exitWith
{
	...
	diag_log format["[ALYSIA:ERROR] Weapon shop %1 not defined in ADENIS_SHOPS_ARMA (class not found)", _type];
};

_side = getText(missionConfigFile >> "ADENIS_SHOPS_ARMA" >> _type >> "side");
if ((_side != "") && (str(playerSide) != _side)) exitWith
{
	[format[
		...
		getText(missionConfigFile >> "ADENIS_SHOPS_ARMA" >> _type >> "name")
...
} forEach getArray(missionConfigFile >> "ADENIS_SHOPS_ARMA" >> _type >> "stocks");
