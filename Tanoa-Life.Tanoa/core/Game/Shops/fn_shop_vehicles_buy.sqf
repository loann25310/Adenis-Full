...
if (_sel isEqualTo -1) exitWith {
	["Vous n'avez pas sélectionné de véhicule à acheter."] call AdenisClient_fnc_error;
};

_className = lbData[2303, (lbCurSel 2303)];
if (_className isEqualTo "") exitWith {
	["Impossible de récupérer les informations du véhicule à acheter."] call AdenisClient_fnc_error;
};

_price = getNumber(missionConfigFile >> "ADENIS_VEHICLES" >> _className >> "buyPrice");
if (_price isEqualTo 0) exitWith {
	["Impossible de récupérer le prix du véhicule à acheter."] call AdenisClient_fnc_error;
};
if (_price > g_cash) exitWith {
	["Vous n'avez pas à assez d'argent sur vous."] call AdenisClient_fnc_error;
};
...
if (getNumber(missionConfigFile >> "ADENIS_SHOPS_VEHICLES" >> g_shop_vehicles_type >> "createVehicle") isEqualTo 1) then
{
	...
	if (isNil "_validSpawn") then {
		...
		["Le véhicule vous venez d'acheter a été <t color='#01DF01'>livré</t> au concessionnaire.", "buy"] call AdenisClient_fnc_info;
	};
} else {
	_vehicle = objNull;
	_proceed = true;
	["Le véhicule vous venez d'acheter sera <t color='#01DF01'>disponible</t> dans votre garage d'ici quelques secondes.", "buy"] call AdenisClient_fnc_info;
};
...
if (_proceed) then
{
	...
	[player, _vehicle, _className] remoteExec ["AdenisServer_fnc_vehicleCreate", 2];
};
