...
{
	["La panier est vide."] call AdenisClient_fnc_error;
	g_shop_active = false;
};
ctrlShow[2414, false];
ctrlShow[2415, false];
...
{
	if (_total > 0) then {
		[true, _total] call AdenisClient_fnc_handleCash;
	} else {
		[false, abs(_total)] call AdenisClient_fnc_handleCash;
	};
};
...
if (g_shop_receive_illegal > 0) then
{
	...
	{
		[true, "illegal_money", round (g_shop_receive_illegal / 2)] call AdenisClient_fnc_handleInv;
	}
	else
	{
		if(west countSide playableUnits < 3) then
		{
			[true, "illegal_money", round (g_shop_receive_illegal / 1.5)] call AdenisClient_fnc_handleInv;
		}
		else
		{
			[true, "illegal_money", g_shop_receive_illegal] call AdenisClient_fnc_handleInv;
		};
	};
...
{
	[g_company, true, g_shop_receive_company, (player getVariable "realname"), "Activité professionnelle"] remoteExec ["AdenisServer_fnc_company_bank_handle", 2];
