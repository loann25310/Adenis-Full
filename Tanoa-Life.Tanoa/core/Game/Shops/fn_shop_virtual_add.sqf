...
switch _type do
{
	...
	{
		if (_full) then {
			_amount = [_item, floor((g_maxWeight - g_shop_weight_actual) / ([_item] call AdenisClient_fnc_itemGetWeight)), g_shop_weight_actual, g_maxWeight] call AdenisClient_fnc_calWeightDiff;
		} else {
			_amount = [_item, 1, g_shop_weight_actual, g_maxWeight] call AdenisClient_fnc_calWeightDiff;
		};

		if (_amount > 0) then
		{
			_index = [_item, g_shop_tmp_buy] call AdenisClient_fnc_index;
	...
	case 1:
	{
		...
		if ([false, _item, _amount] call AdenisClient_fnc_handleInv) then
		{
			_index = [_item, g_shop_tmp_sell] call AdenisClient_fnc_index;
			if (_index isEqualTo -1) then {
