...
if ([_license] call AdenisClient_fnc_hasLicense) exitWith {
	(format["%1 a <t color='#01DF01'>refusé</t> votre proposition.<br/>Il possède déjà la licence que vous lui proposez.", player getVariable ["realname", profileName]]) remoteExecCall ["AdenisClient_fnc_info", _from];
};
...
if (_action) then
{
	[format["%1 a <t color='#01DF01'>accepté</t> votre proposition.", player getVariable ["realname", profileName]]] remoteExecCall ["AdenisClient_fnc_info", _from];
	["Vous avez <t color='#01DF01'>accepté</t> la proposition."] call AdenisClient_fnc_info;
	missionNamespace setVariable [format["license_%1", _license], true];
} else {
	(format["%1 a <t color='#01DF01'>refusé</t> votre proposition.", player getVariable ["realname", profileName]]) remoteExecCall ["AdenisClient_fnc_info", _from];
	["Vous avez <t color='#DF0101'>refusé</t> la proposition."] call AdenisClient_fnc_info;
};
