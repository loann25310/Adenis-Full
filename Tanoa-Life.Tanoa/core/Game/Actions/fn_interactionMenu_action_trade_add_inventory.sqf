...
if (_sel isEqualTo -1) exitWith {
	["Vous n'avez pas sélectionné d'objet dans la liste."] call AdenisClient_fnc_error;
};

_item = lbData[10019, _sel];
if (_item isEqualTo "") exitWith {
	["Impossible de récupérer l'objet que vous avez sélectionné."] call AdenisClient_fnc_error;
};

g_interaction_trade_active = true;

if (_type) then {
	_amount = [_item] call AdenisClient_fnc_itemCount;
} else {
	_amount = 1;
};
...
{
	_index = [_item, g_interaction_trade_inventory] call AdenisClient_fnc_index;
