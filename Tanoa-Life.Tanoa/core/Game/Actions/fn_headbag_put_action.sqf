...
{
	if (["mgsr_headbag", false] call AdenisClient_fnc_handleItem) then
	{
		g_action_inUse = true;
		player playMove "AinvPercMstpSnonWnonDnon_Putdown_AmovPercMstpSnonWnonDnon";
		waitUntil {((animationState player) isEqualTo "ainvpercmstpsnonwnondnon_putdown_amovpercmstpsnonwnondnon")};
		[] remoteExec ["AdenisClient_fnc_headbag_put_target", _target];
		g_action_inUse = false;
	} else {
		["Vous n'avez pas de cagoule."] call AdenisClient_fnc_info;
	};
} else {
