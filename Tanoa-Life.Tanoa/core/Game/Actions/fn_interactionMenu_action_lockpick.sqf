...
if (!(_target getVariable ["restrained", false])) exitWith {
	["La cible n'est pas menottée."] call AdenisClient_fnc_error;
};
if (!("Alysia_Lockpick" in (magazines player))) exitWith {
	["Vous n'avez pas de kit de crochetage."] call AdenisClient_fnc_error;
};

if (!(["Crochetage de menottes", 7, _target, "", "AinvPknlMstpsnonWnonDnon_medic_1"] call AdenisClient_fnc_showProgress)) exitWith {};
...
if ((random(100)) < 35) then
{
	...
	["Crochetage <t color='#3ADF00'>réussi</t> !<br/>La cible n'est maintenant plus menottée."] call AdenisClient_fnc_info;
} else {
