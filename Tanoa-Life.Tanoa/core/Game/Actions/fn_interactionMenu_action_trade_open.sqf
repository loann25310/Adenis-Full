...
while {!(isNull (findDisplay 10000))} do
{
	if (isNull _target) exitWith
	{
		["Echange <t color='#FFBF00'>interrompu</t>.<br/>Cible invalide."] call AdenisClient_fnc_info;
		closeDialog 0;
	};
	if (player getVariable ["is_coma", false]) exitWith
	{
		["Echange <t color='#FFBF00'>interrompu</t>.<br/>Vous êtes dans le coma."] call AdenisClient_fnc_info;
		closeDialog 0;
	};
	if ((player distance _target) > ((((boundingBox _target) select 1) select 0) + 2.5)) exitWith
	{
		["Echange <t color='#FFBF00'>interrompu</t>.<br/>Vous êtes trop loin de la cible."] call AdenisClient_fnc_info;
		closeDialog 0;
	};
	if (player getVariable ["restrained", false]) exitWith
	{
		["Echange <t color='#FFBF00'>interrompu</t>.<br/>Vous êtes menotté."] call AdenisClient_fnc_info;
		closeDialog 0;
	};
	if (player getVariable ["surrender", false]) exitWith
	{
		["Echange <t color='#FFBF00'>interrompu</t>.<br/>Vous avez les mains sur la tête."] call AdenisClient_fnc_info;
		closeDialog 0;
	};
	if (_target getVariable ["is_coma", false]) exitWith
	{
		["Echange <t color='#FFBF00'>interrompu</t>.<br/>La cible est dans le coma."] call AdenisClient_fnc_info;
		closeDialog 0;
	};
	if (_target getVariable ["restrained", false]) exitWith
	{
		["Echange <t color='#FFBF00'>interrompu</t>.<br/>La cible est menotté."] call AdenisClient_fnc_info;
		closeDialog 0;
	};
	if (_target getVariable ["surrender", false]) exitWith
	{
		["Echange <t color='#FFBF00'>interrompu</t>.<br/>La cible a les mains sur la tête."] call AdenisClient_fnc_info;
		closeDialog 0;
	};
