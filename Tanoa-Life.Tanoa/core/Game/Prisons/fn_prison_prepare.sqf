...
if (g_arrest_Gear isEqualTo []) then {
	g_arrest_Gear = [g_cash, ([player] call AdenisClient_fnc_getGear), ([player] call AdenisClient_fnc_getInv)];
};
...
[true, "prison_food", 3] call AdenisClient_fnc_handleinv;

[] spawn AdenisClient_fnc_prison_loop;
