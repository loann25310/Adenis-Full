...
if (!(_object getVariable ["destroyed", false])) exitWith {
	["Le haut parleur n'est pas saboté."] call AdenisClient_fnc_error;
};

_repair_item = "ironp";
_repair_amount = 2;

if (([_repair_item] call AdenisClient_fnc_itemCount) < _repair_amount) exitWith
{
	[
		format
		[
			...
			_repair_amount,
			[_repair_item] call AdenisClient_fnc_itemGetName
		]
	] call AdenisClient_fnc_error;
};

if (["Réparation d'un haut parleur", 10, _object, "", "AinvPknlMstpsnonWnonDnon_medic_1"] call AdenisClient_fnc_showProgress) then
{
	if (!(_object getVariable ["destroyed", false])) exitWith {
		["Le haut parleur n'est pas saboté."] call AdenisClient_fnc_error;
	};

	if ([false, _repair_item, _repair_amount] call AdenisClient_fnc_handleInv) then
	{
		[
			format
			[
