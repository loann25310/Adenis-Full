...
while {!(isNull (findDisplay 91000))} do
{
	...
	{
		["Transfert annulé : Vous êtes plusieurs à intéragir avec l'inventaire."] call AdenisClient_fnc_error;
		closeDialog 0;
	};
	if (player getVariable ["restrained", false]) exitWith
	{
		["Transfert annulé : Vous êtes menotté."] call AdenisClient_fnc_error;
		closeDialog 0;
	};
	if (player getVariable ["surrender", false]) exitWith
	{
		["Transfert annulé : Vous avez les mains sur la tête."] call AdenisClient_fnc_error;
		closeDialog 0;
	};
