...
if ((_inUse != "") && !(_inUse isEqualTo (getPlayerUID player))) exitWith {
	["Le coffre est déjà en cours d'utilisation."] call AdenisClient_fnc_error;
};

if (!(["Accès au stockage", round(random(2) + 0.5), _target] call AdenisClient_fnc_showProgress)) exitWith {};

_weight = [_target] call AdenisClient_fnc_getVehicleWeightMax;
if (_weight isEqualTo 0) exitWith {
	["Impossible de déterminer l'inventaire maximum de la cible."] call AdenisClient_fnc_error;
};
...
while {!(isNull _display)} do
{
	...
	{
		["Fouille du stockage <t color='#FFBF00'>interrompu</t>.<br/>Cible invalide."] call AdenisClient_fnc_info;
		closeDialog 0;
	};
	if ((player distance g_interaction_target) > _distance) exitWith
	{
		["Fouille du stockage <t color='#FFBF00'>interrompu</t>.<br/>Vous êtes trop loin de la cible."] call AdenisClient_fnc_info;
		closeDialog 0;
	};
	...
	{
		["Fouille du stockage <t color='#FFBF00'>interrompu</t>.<br/>Quelqu'un d'autre fouille le même stockage que vous."] call AdenisClient_fnc_info;
		closeDialog 0;
	};
	if (((locked g_interaction_target) isEqualTo 2) && !(g_interaction_target in g_vehicles)) exitWith
	{
		["Fouille du stockage <t color='#FFBF00'>interrompu</t>.<br/>Le stockage est vérrouillé et vous ne possédez pas les clefs."] call AdenisClient_fnc_info;
		closeDialog 0;
	};
	if (player getVariable ["is_coma", false]) exitWith
	{
		["Fouille du stockage <t color='#FFBF00'>interrompu</t>.<br/>Vous êtes dans le coma."] call AdenisClient_fnc_info;
		closeDialog 0;
	};
	if (player getVariable ["restrained", false]) exitWith
	{
		["Fouille du stockage <t color='#FFBF00'>interrompu</t>.<br/>Vous êtes menotté."] call AdenisClient_fnc_info;
		closeDialog 0;
	};
	if (player getVariable ["surrender", false]) exitWith
	{
		["Fouille du stockage <t color='#FFBF00'>interrompu</t>.<br/>Vous avez les mains sur la tête."] call AdenisClient_fnc_info;
		closeDialog 0;
	};
...
[true] call AdenisClient_fnc_temporyInventory_update;

if (_playSound) then {
	[g_interaction_target, "trunk_close", 20] call AdenisClient_fnc_globalSay3d;
};
