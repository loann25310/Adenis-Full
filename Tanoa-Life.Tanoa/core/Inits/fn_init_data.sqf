...
[(_this select 9), (_this select 10), (_this select 11)] spawn
{
	...
	if (_this select 2) then {
		[] spawn AdenisClient_fnc_handleComa;
	} else {
		[((4000 - (_this select 0)) * -1)] call AdenisClient_fnc_handleBlood;
		[(_this select 1)] call AdenisClient_fnc_handleBleed;
	};
};
...
[(_this select 38)] call AdenisClient_fnc_handleAlcool;
...
// ======== INVENTORY ARMA ===========
[(_this select 45)] call AdenisClient_fnc_loadGear;
...
if (count(_inv) > 0) then
{
	...
	if (!(isNil "_tmp")) then
	{
	 	...
	 	{
			[profileName, getPlayerUID player, _inv, _tmp] remoteExecCall ["AdenisServer_fnc_logTemporyInventory", 2];
			[true] call AdenisClient_fnc_temporyInventory_reset;
	 	};
	};
};
g_maxWeight = 24;
