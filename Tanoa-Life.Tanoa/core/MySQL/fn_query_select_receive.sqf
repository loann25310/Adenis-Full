...
{
	["Une erreur est survenue lors du chargement de vos données. Veuillez vous reconnecter ou contacter un administrateur si le problème persiste"] spawn AdenisClient_fnc_errorExit;
	false;
};
if (!((_basic select 1) isEqualTo "") && !((_basic select 1) isEqualTo profileName)) exitWith
{
	[format["Vous n'êtes pas autorisé à changer d'identité. Votre pseudo actuel [%1] ne correspond pas à celui que vous aviez lors de votre inscription [%2].", profileName, (_basic select 1)]] spawn AdenisClient_fnc_errorExit;
	false;
};
if ((face player) != (_basic select 37)) exitWith
{
	[format["Vous n'êtes pas autorisé à changer de visage. Votre visage actuel [%1] ne correspond pas à celui que vous aviez lors de votre inscription [%2].", (face player), (_basic select 37)]] spawn AdenisClient_fnc_errorExit;
	false;
};
if (
	...
	["Ce slot est réservé aux membres de l'administration."] spawn AdenisClient_fnc_errorExit;
	false;
};
...
_allowed = switch (playerSide) do
{
	case civilian:
	{
		...
		{
			["Vous n'êtes pas autorisé à changer de faction"] spawn AdenisClient_fnc_errorExit;
			false;
		};
/*
		...
		{
			["Ce slot est réservé aux donateurs"] spawn AdenisClient_fnc_errorExit;
			false;
		};
*/
		true;
	...
	case west:
	{
		...
		{
			[format["Ce slot est réservé aux membres de la faction : %1", [west] call AdenisClient_fnc_sideToStr]] spawn AdenisClient_fnc_errorExit;
			false;
		};
...
*/
	...
	case east:
	{
		...
		{
			[format["Ce slot est réservé aux membres de la faction : %1", [east] call AdenisClient_fnc_sideToStr]] spawn AdenisClient_fnc_errorExit;
			false;
		};
...
*/
	...
	case independent:
	{
		...
		{
			[format["Ce slot est réservé aux membres de la faction : %1", [independent] call AdenisClient_fnc_sideToStr]] spawn AdenisClient_fnc_errorExit;
			false;
		};
...
if (isNil "_respawn") then {
	...
	if (isNull _house) then {
		[] call AdenisClient_fnc_house_menu_action_spawn_cancel;
	} else {
...
if (!(isNull g_laboratory)) then
{
	if (g_laboratory getVariable ["construction", false]) then
	{
		...
		if (isClass _config_marker) then
		{
			...
			_marker setMarkerTextLocal ([configName _config] call AdenisClient_fnc_itemGetName);
		};
	};
};
