...
if (isClass(missionConfigFile >> "ADENIS_ITEMS" >> _item)) then
{
	if (isClass(missionConfigFile >> "ADENIS_ITEMS" >> _item >> "market")) then {
		_ret = [_item] call AdenisClient_fnc_market_getPrice;
	} else {
		_ret = getNumber(missionConfigFile >> "ADENIS_ITEMS" >> _item >> "price_sell");
		if (_ret isEqualTo 0) then {
			systemChat format["[ERROR] Item price %1 not defined in ADENIS_ITEMS or isEqualTo 0", _item];
		};
	};
} else {
