...
if (g_firstCombatActive) exitWith {
	["Vous ne pouvez pas saisir alors que vous êtes en combat."] call AdenisClient_fnc_error;
};
...
if (_display) then {
	[format["Vous avez saisi %1 objet%2", _objCount, if (_objCount > 1) then {"s"} else {""}]] call AdenisClient_fnc_info;
};
