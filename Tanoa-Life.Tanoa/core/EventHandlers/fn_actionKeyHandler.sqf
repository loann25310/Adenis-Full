...
if (_vehicle isKindOf "Man") then
{
...

	{
		if (isPlayer _x) then
		{
			if (_x getVariable ["is_coma", false]) then {
				[_x, true] spawn AdenisClient_fnc_action_body_drop;
			} else {
				[_x, true] spawn AdenisClient_fnc_stopescort;
			};
			true breakOut "main";
		} else {
	...
	if (!(isNull _target)) then
	{
		...
		if ((player distance _target) < ((((boundingBox _target) select 1) select 0) + 2.5)) then
		{
			if (isPlayer _target) then
			{
				if ((_target isKindOf "Car") || (_target isKindOf "Ship") || (_target isKindOf "Air") || (_target isKindOf "Tank")) then
				{
					...
					{
						[(vehicle _target)] call AdenisClient_fnc_interactions_player_to_vehicle;
						true breakOut "main";
					};
				} else {
					if (_target getVariable ["is_coma", false]) then {
						[_target] call AdenisClient_fnc_interactions_player_to_player_coma;
					} else {
						[_target] call AdenisClient_fnc_interactions_player_to_player_basics;
					};
					true breakOut "main";
				};
			} else {
				...
				if (
					...
					[_target] call AdenisClient_fnc_interactions_player_to_vehicle;
					true breakOut "main";
				};

				if (_type isEqualTo "Skyline_Alysia_Crane_01_F") then
				{
					[_target] call AdenisClient_fnc_interactions_player_to_skull;
					true breakOut "main";
				};

				if (isClass(missionConfigFile >> "ADENIS_ATM" >> _type)) then
				{
					[_target] call AdenisClient_fnc_interactions_player_to_atm;
					true breakOut "main";
				};

				if (isClass(missionConfigFile >> "ADENIS_STORAGES" >> _type)) then
				{
					[_target, "Trunk", true, true, false, false] spawn AdenisClient_fnc_virtual_menu_exhange_open;
					true breakOut "main";
				};

				if (_type isEqualTo "Turtle_F") then
				{
					[_target] call AdenisClient_fnc_catchTurtle;
					true breakOut "main";
				};

				if (isClass(missionConfigFile >> "ADENIS_FARMING_PLANT_OBJETCS" >> _type)) then
				{
					[_target] spawn AdenisClient_fnc_plantHarvest;
					true breakOut "main";
				};

				if (count(_target getVariable ["company_info", []]) > 0) then
				{
					[_target] call AdenisClient_fnc_interactions_player_to_company;
					true breakOut "main";
				};

				if (count(_target getVariable ["laboratory_info", []]) > 0) then
				{
					[_target] call AdenisClient_fnc_interactions_player_to_laboratory;
					true breakOut "main";
				};

				if ((_type in ["Alsatian_Sand_F", "Alsatian_Black_F", "Alsatian_Sandblack_F"]) && (alive _target)) then
				{
					[_target] call AdenisClient_fnc_interactions_player_to_dog;
					true breakOut "main";
				};

				if (isClass(missionConfigFile >> "ADENIS_FUEL_STATION" >> _type)) then
				{
					[_target] call AdenisClient_fnc_interactions_player_to_station;
					true breakOut "main";
				};

				if (isClass(missionConfigFile >> "ADENIS_CHAIRS" >> _type)) then
				{
					[_target] call AdenisClient_fnc_interactions_player_to_chair;
					true breakOut "main";
				};
				...
				if (isClass(missionConfigFile >> "ADENIS_DYN_OBJECTS" >> _type)) then
				{
					...
					{
						[_target] spawn AdenisClient_fnc_dynamicObject_packup;
						true breakOut "main";
					};
				};
				...
				if (_type isEqualTo "Land_Suitcase_F") then
				{
					...
					{
						[_target] spawn AdenisClient_fnc_moneyBag;
						true breakOut "main";
					};
				};
				...
				{
					[_target] spawn AdenisClient_fnc_c4_open;
					true breakOut "main";
				};

				if (_type isEqualTo "Bank_Drill") then
				{
					[_target] call AdenisClient_fnc_interactions_player_to_drill;
					true breakOut "main";
				};

				if (_type isEqualTo "Skyline_Alysia_Boite_01_F") then
				{
					[_target, "items", true, true, false, false, true] spawn AdenisClient_fnc_virtual_menu_exhange_open;
					true breakOut "main";
				};
			};
		};
		...
		{
			if (((player distance _target) < 10) || (([_target] call AdenisClient_fnc_nearestDoor) != 0)) then
			{
				[_target] call AdenisClient_fnc_interactions_player_to_house;
				true breakOut "main";
			};
		};
		...
		if ((player distance _target) < ((((boundingBox _target) select 1) select 0)) + 1.2) then
		{
			...
			{
				[_target] spawn AdenisClient_fnc_farm_object;
				true breakOut "main";
			};
		};
	};
	...
	if (!(isNil "_suit")) then
	{
		[_suit, "items", true, true, false, false, true] spawn AdenisClient_fnc_virtual_menu_exhange_open;
		true breakOut "main";
	};
	...
	{
		[_skull] call AdenisClient_fnc_interactions_player_to_skull;
		true breakOut "main";
	};
	...
	{
		[_plant] spawn AdenisClient_fnc_plantHarvest;
		true breakOut "main";
	};
...

	{
		...
		{
			[format["Vous avez découvert <t color='#74DF00'>%1</t>.", (markerText (configName _x))]] call AdenisClient_fnc_info;
			[(configName _x)] call AdenisClient_fnc_dynamicMarkers_reveal;
			true breakOut "main";
		} else {
			if (str(playerSide) in getArray(_x >> "destroy")) then
			{
				[(configName _x)] spawn AdenisClient_fnc_dynamicMarkers_destroy;
				true breakOut "main";
			};
		};
...

	{
		...
		{
			[(configName _x)] spawn AdenisClient_fnc_plantSeed;
			true breakOut "main";
		};
...

	{
		...
		if (getNumber(_x >> "water") isEqualTo 0) then
		{
			...
			{
				[_marker] spawn AdenisClient_fnc_pickGather;
				true breakOut "main";
			};
		} else {
	...
	if (!isNull(g_company)) then
	{
		if (((g_company getVariable "company_info") select 2) isEqualTo "farming") then
		{
			...
			{
				[] spawn AdenisClient_fnc_plantSeed;
				true breakOut "main";
			};
		};
	};
	...
	if (alive _vehicle) then
	{
		...
		{
			[_vehicle] call AdenisClient_fnc_interactions_player_to_vehicle;
			true breakOut "main";
		};
	};
};
