...
if (hasInterface) then
{
	...
	{
		["Le serveur redémarre dans moins de 4 minutes, veuillez vous reconnecter après."] spawn AdenisClient_fnc_errorExit;
	};
	...
	if (_check != "") exitWith
	{
		[format["Le pbo [%1] est nécessaire pour vous connecter.", _check]] spawn AdenisClient_fnc_errorExit;
	};
	...
	if (((getResolution) select 5) != 0.55) exitWith
	{
		...
		] spawn AdenisClient_fnc_errorExit;
	};
	...
	[] call AdenisClient_fnc_init_variables;
	...
	if (!_staff) then
	{
		...
		waitUntil {(["AdenisRP x Your-dev.fr", (call TFAR_fnc_getTeamSpeakServerName)] call BIS_fnc_inString)};
	...
	[] call AdenisClient_fnc_init_eventHandlers;
	[] call AdenisClient_fnc_init_keys;
	...
	[] call AdenisClient_fnc_init_buildings;
	...
	[player, profileNamespace getVariable "temp_inventory"] remoteExec ["AdenisServer_fnc_query_select_connect", 2];
	...
	g_session_query call AdenisClient_fnc_init_data;
	...
	[] call AdenisClient_fnc_init_loops;
	...
	[(g_session_query select 12), (g_session_query select 13), (g_session_query select 14)] call AdenisClient_fnc_init_position;
	...
	player addAction
	[
		"<t color='#006eff'><img image='\a3\ui_f\data\IGUI\Cfg\Actions\take_ca'/></t> Jeter l'ancre",
		{
	...
	[] spawn {
		...
		_marker = createMarkerLocal ["Marker_Moi_LAST", player];
		_marker setMarkerTypeLocal "Lyeed_GPS";
		_marker setMarkerColorLocal "ColorCIV";
		_marker setMarkerTextLocal "Moi";
	    while {true} do {
	        _marker setMarkerPosLocal (position player);
			_marker setMarkerDirLocal ((getDir player) - 45);
